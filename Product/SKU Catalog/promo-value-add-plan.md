...
## 0. Doctrine — offer instruments, never discounts
...
| Fact | Detail |
|---|---|
| Construction | soft PVC 2D+2D keychain, two pieces: **Mé sheep mascot charm 45×40mm×5mm** (2-color front + 1-color back, loop on top) + **wordmark tag 40×12mm** (deep dark brownish-green wordmark) |
...
## 2. Placement map — where each instrument sits on the ladder
...
| P6 | **Subscription — recurring perk** | eye gels | gels in **every 3rd delivery** on 24/48 tiers; 48-tier ("inner circle") also gets **first access to each drop's keychain colorway** | $3.25/3 ≈ **$1.08/delivery amortized** | deepens perks ladder without touching the 20% discount cap; configured as a Loop gift rule |
| P7 | **Drop-day GWP** | eye gels | drop-open window only: **spend $66+ → gels free** ($66 = the 24-pack, the CFO's "margin workhorse") | $3.25/qualifying order | pushes AOV to the 24-pack exactly like the $50 free-ship threshold does — twin nudges, same target |
...
|---|---|---|---|
| 10% discount | $6.60 | $27.44 | 41.6% ✅ |
| **gels GWP at $66+ (P7)** | **$3.25** | **$30.79** | **46.7% ✅** |

Referral ladder swap (general program, P3/P4): all-credit ladder costs $30 per 5-referral advocate; the physical ladder costs **$9.75** — with the friend-side 15% code (~$6.12), blended acquisition cost falls from ~$12 to **~$8.07**, comfortably under the $9 launch-mix CAC ceiling.

SMS placement (P1) forward look: today SMS earns SHROOME30 over SHROOME20 — those extra 10 points cost **$3.50 of contribution** on a 12-pack ($9.17 → $5.67, and SHROOME30 already sits below the 30% CM floor per the CFO memo). SHROOME30 is committed for the launch window and stays. **Post-launch (at the CFO's anchor review), the recommended structure is: SMS = 10-minute early access + keychain in first box + a code no deeper than 22%** — the object replaces the margin-breaking code depth. Decision rides with the $40–42 anchor decision, end of launch month.

Margin-floor audit: every placement adds $1.08–3.25 of COGS to orders whose Base contribution is $8.62–34.04. The only cells that need watching are the deepest sub cells (48/monthly at $8.62 contribution + $1.08 gels amortized = $7.54, ~26% CM — **below floor, same as before the gels; the gels worsen it by ~4pts**). Rule: **P6 gels ride on 24/48 tiers, and if real COGS come in above $0.90/sachet, P6 drops to every 4th delivery before we touch anything else.**

## 4. Scarcity & collectible mechanics
...
2. **Gift injection — three lanes:**
   ...
   - **Referral / draw / win-back (P2, P3, P4, P8, P9):** rewards app (Smile.io/ReferralCandy per roadmap Phase 4) custom-reward webhook → if the customer has a pending/next order, add the $0 line via Admin API order edit or subscription one-time add-on; else create a **$0 draft order** (tag `promo-reward`, `kch-3ref` etc.) for flat-mail fulfillment. Until the rewards app is live, this is a weekly manual draft-order batch from a Klaviyo segment export — acceptable at launch scale.
3. **Inventory discipline.** $0 line items still decrement inventory — that's the point: promo stock is finite (1,000pc test run) and placements P1–P9 draw from the same pool. 3PL bins per colorway lot; Code-128 SKU labels (`SHR-KCH-01`, `SHR-EYG-06`) for pick verification; the poly bag's printed GTIN barcode doubles as receiving verification. When keychain inventory hits its drop allocation floor, P2 draws pause first, then P1 — **referral milestones (P3/P8) are never shorted; they're earned debts.**
4. **Reporting.** Order tags per placement (`gift-sms-001`, `gift-sub-first`, `gwp-drop-002`, `ref-3-kch`, `ref-5-eyg`, `winback-eyg`) so the CFO can see promo COGS by placement against the contribution table in §3. Klaviyo profile properties `kch_owned: true`, `kch_colorways: [...]` feed later collab/collector flows.

### 1,000pc test-run allocation (proposal — adjust to real drop-002 forecast)
...
## 6. Open items to verify (blockers marked ⚠️)

1. ⚠️ Real landed costs (keychain and gels) → re-run §3 tables; assumptions break if keychain lands >$2.50 or gels >$5.
2. ⚠️ Gels: pairs vs singles + final INCI → gates all "uses" and ingredient copy in the creative pack.
3. Factory colorway options vs brand palette → colorway naming plan.
4. CFO sign-off on the §2/§3 referral-ladder amendment to `discount-matrix.md` §4 (it strictly lowers cost, but the matrix is his document).
