# TikTok / Reels Video Scripts

> Last updated: March 2026
> Format: Short-form vertical video (9:16)
> Duration: 15-30 seconds each
> Core visual motif: Tear, pour, done

---

## Script 1: "The 15-Second Matcha Latte" (ASMR)

**Pillar:** Lifestyle / Energy
**Duration:** 15 seconds
**Style:** No voiceover. Pure ASMR. All sound design.

### Hook (0-2s)
Close-up macro shot of fingers gripping the sachet notch. The tear sound is crisp and loud.

### Shot List

| Time | Shot | Audio |
|------|------|-------|
| 0-2s | Extreme close-up: fingers tear sachet along the perforation. Liquid visible inside. | Amplified tear sound. No music yet. |
| 2-5s | Overhead: liquid streams out of sachet into a clear glass filled with oat milk. Slow, controlled pour. | Soft liquid-into-milk sound. Gentle ambient tone fades in. |
| 5-9s | Side angle through glass: matcha plume blooms and swirls through the milk. Green ribbons curling downward. No stirring yet. | Lo-fi ambient pad builds slightly. |
| 9-12s | Close-up: a single slow stir with a spoon. The drink turns from streaked to uniform sage green. | Spoon-against-glass clink. |
| 12-15s | Beauty shot: glass on a sunlit surface, condensation on the outside, hand reaches in and lifts it. Hold for 2 beats. | Music resolves. Silence on the sip. |

### Text Overlays
- (0s) — *nothing. let the tear speak.*
- (5s) — "15 seconds."
- (7s) — "No whisk. No blender."
- (12s) — "Cafe-grade matcha."
- (14s) — "shroome" + logo

### Music
Lo-fi ambient / soft piano. Think "study beats but make it elegant." No vocals. Keep it quiet enough that ASMR sounds dominate.

### Hashtags
`#matcha #matchalatte #asmr #matchaasmr #morningroutine #matchatok #oatmilk #shroome #functionalfood #nowhiskneeded`

---

## Script 2: "What's Actually in Your Mushroom Coffee?" (Education/Hook)

**Pillar:** Product Education / Transparency
**Duration:** 25 seconds
**Style:** Direct-to-camera or voiceover with B-roll cutaways

### Hook (0-2s)
Face fills the frame. Deadpan delivery, slightly raised eyebrow.

**VO:** "Your mushroom coffee probably has less than 5% actual mushroom."

### Shot List

| Time | Shot | Audio |
|------|------|-------|
| 0-2s | Direct-to-camera or text-on-screen hook. Bold white text on dark background if no face. | VO delivers hook. |
| 2-7s | Hand holds a small pile of brown rice grains. Text overlay labels it. Camera pushes in. | VO: "Most brands use mycelium grown on grain. That means you're mostly drinking rice starch." |
| 7-12s | Cut to: hand holds a whole dried lion's mane mushroom (or reishi). Rotate it slowly. Beautiful texture. | VO: "Fruiting body is the actual mushroom. The part with the compounds that matter." |
| 12-17s | Close-up of shroome sachet back label. Finger points to "fruiting body extract" and beta-glucan percentage. | VO: "We use dual-extracted fruiting body. Seventy percent-plus beta-glucans. It's on the label." |
| 17-22s | Quick tear-pour-done sequence with the shroome sachet into a latte. Beauty shot. | VO: "Real mushrooms. Real matcha. That's it." |
| 22-25s | End card: shroome logo + "Link in bio" | Music resolves. |

### Text Overlays
- (0s) — "Your mushroom coffee has a problem"
- (3s) — "MYCELIUM ON GRAIN = mostly rice"
- (8s) — "FRUITING BODY = the real mushroom"
- (13s) — "70%+ beta-glucans" (highlight on label)
- (22s) — "Link in bio"

### Music
Trending editorial sound or subtle tension-building beat. Something that feels like "exposing the industry" without being aggressive.

### Hashtags
`#mushroomcoffee #lionsmane #reishi #betaglucans #functionalfoods #supplementtok #matchalatte #shroome #fruitingbody #healthtok #readthelabel`

---

## Script 3: "Morning Routine: Before vs After" (Lifestyle)

**Pillar:** Lifestyle / Energy
**Duration:** 25 seconds
**Style:** Split narrative. Same person, same morning, two realities. Slightly comedic on the left, aspirational on the right.

### Hook (0-2s)
Split screen or quick cut. Left side is chaotic. Right side is calm. The contrast is immediate.

### Shot List

| Time | Side | Shot | Audio |
|------|------|------|-------|
| 0-2s | BEFORE | Alarm blares. Hand slams phone. Person groans into pillow. | Harsh alarm sound. |
| 2-5s | BEFORE | Stumbling to kitchen. Opens cabinet. Pulls out French press, grinder, filters. Counter is cluttered. | Clumsy sounds. Grinder buzz. |
| 5-8s | BEFORE | Pouring coffee, spills on counter. Paper towels. Checks time. Panicked face. | Splashing, muttering. |
| 8-10s | — | HARD CUT TO BLACK. Text: "same morning. different ritual." | Beat drop or silence. |
| 10-13s | AFTER | Same alarm. Hand calmly picks up phone. Stretches. Soft light. | Gentle tone. Calm music begins. |
| 13-17s | AFTER | Kitchen is clean. One sachet on the counter. Tears it open. Pours into a glass of cold oat milk. Done. | ASMR tear. Pour. Single stir. |
| 17-21s | AFTER | Sipping the latte by the window. Calm face. Puts on jacket. | Music continues, warm. |
| 21-25s | AFTER | Walks out the door, put-together, on time. Latte in hand. | Door closes softly. |
| 25s | — | End card: "shroome" logo. | Music resolves. |

### Text Overlays
- (0s) — "POV: your morning before shroome"
- (8s) — "same morning. different ritual."
- (17s) — "cafe energy. home address."
- (24s) — "shroome" + logo

### Music
Before section: no music or chaotic/comedic sound. After section: warm lo-fi or soft acoustic guitar. The contrast in audio is as important as the visual.

### Hashtags
`#morningroutine #beforeandafter #matchalatte #coffeeaddict #shroome #morningmotivation #routinetok #matchatok #healthyhabits #oatmilklatte`

---

## Script 4: "I Replaced Coffee for 30 Days" (Results/Data)

**Pillar:** Product Education / Lifestyle
**Duration:** 30 seconds
**Style:** Vlog/journal format. Each "day" is a quick cut with the same making-the-latte ritual as a visual anchor. Text-heavy with VO.

### Hook (0-2s)
Close-up of coffee mug being pushed aside. Shroome sachet slides into frame.

**VO / Text:** "I replaced coffee with mushroom matcha for 30 days."

### Shot List

| Time | Day | Shot | VO / Text Overlay |
|------|-----|------|-------------------|
| 0-2s | — | Coffee mug pushed aside. Shroome sachet enters frame. | "I replaced coffee for 30 days." |
| 2-7s | Day 1 | Tearing sachet, pouring into milk. Skeptical face to camera. | "Day 1. I'm skeptical. This is weird." |
| 7-12s | Day 7 | Same ritual, more fluid now. Person working at desk mid-afternoon, alert. | "Day 7. No crash at 2pm for the first time in years." |
| 12-18s | Day 14 | Making the latte. Cut to phone showing Oura Ring sleep score trending up. | "Day 14. Sleeping better. My Oura Ring confirms it." |
| 18-24s | Day 30 | Confident pour. Smiling. Walking outside with latte, energized. | "Day 30. I don't miss coffee. I didn't expect that." |
| 24-27s | — | Quick montage: all four pours back-to-back, fast cuts. | "Same ritual. Every morning." |
| 27-30s | — | End card: beauty shot of finished latte. Logo. | "shroome. Link in bio." |

### Text Overlays
- (0s) — "I quit coffee for 30 days"
- (3s) — "DAY 1" (large, bold)
- (7s) — "DAY 7" + "no 2pm crash"
- (13s) — "DAY 14" + "sleep score: up 12%"
- (19s) — "DAY 30" + "I don't miss it"
- (27s) — "shroome" + "Link in bio"

### Music
Building track that starts mellow and gets slightly more upbeat as the days progress. Momentum without hype.

### Compliance Note
This is personal experience content, not a clinical claim. If posted by the brand account, include "individual results may vary" in caption or as small text overlay on Day 14/30 frames.

### Hashtags
`#quittingcoffee #30daychallenge #mushroommatcha #matchalatte #shroome #nocrash #ouaring #sleephealth #energyboost #healthtok #functionalfoods`

---

## Script 5: "The Ingredient Label Test" (Trust/Transparency)

**Pillar:** Transparency
**Duration:** 25 seconds
**Style:** Direct-to-camera or hands-only with product labels. Confident, educational, slightly confrontational energy. "I'm not mad, I'm just informed."

### Hook (0-2s)
Hold up a supplement bottle (generic, no brand visible). Skeptical look.

**VO:** "Three things to check before you buy any functional beverage."

### Shot List

| Time | Check | Shot | VO |
|------|-------|------|----|
| 0-2s | — | Hand holds generic supplement container. Direct look at camera. | "Three things to check before you buy any functional beverage." |
| 2-8s | #1 | Finger on a label. Zoom in on ingredient list. Circle or highlight "mycelium" vs "fruiting body." | "Number one. Does it say fruiting body or mycelium? Mycelium on grain is mostly filler." |
| 8-14s | #2 | Different angle on label. Looking for beta-glucan percentage. | "Number two. Is there a beta-glucan percentage? If they don't list it, they're probably hiding a low number." |
| 14-19s | #3 | Close-up on matcha grade callout (or lack of one). | "Number three. Ceremonial or culinary grade matcha? Culinary is for baking. You're drinking this." |
| 19-23s | — | Cut to shroome sachet. Flip to back label. All three checkmarks visible. | "If you can't find these answers, you're drinking marketing. Not function." |
| 23-25s | — | Tear, pour, done. Quick beauty shot. Logo. | "shroome. Every answer on the label." |

### Text Overlays
- (0s) — "CHECK YOUR LABEL"
- (3s) — "1. Fruiting body or mycelium?"
- (9s) — "2. Beta-glucan % listed?"
- (15s) — "3. Ceremonial or culinary grade?"
- (19s) — "shroome passes all three."
- (23s) — "Link in bio"

### Music
Clean, editorial beat. Something that says "I did the research so you don't have to." No vocals.

### Hashtags
`#readthelabel #supplementtok #functionalfoods #mushroomcoffee #matchalatte #shroome #betaglucans #fruitingbody #ceremonialmatcha #healthtok #transparency`

---

## Production Notes (All Scripts)

**Filming specs:**
- Vertical 9:16 (1080x1920 minimum, 4K preferred)
- Natural light preferred. No harsh overhead fluorescents.
- Product should always be in warm, real-life settings (kitchen counter, desk, windowsill). No studio white backgrounds.

**Brand consistency:**
- The tear-pour-done sequence appears in every script. This is the signature visual. Film it the same way each time so it becomes recognizable.
- Use the actual shroome sachet. No mockups.
- Oat milk is the default pairing in all videos. Almond or regular milk are acceptable alternates.

**Posting cadence:**
- One script per week. Stagger across TikTok (post first) and Instagram Reels (post 24-48 hours later).
- Post TikTok between 7-9am or 6-8pm local time for highest engagement.

**Caption formula:**
Hook line from script + one sentence of value + CTA ("Link in bio" or "Save this for later") + hashtags.

**Compliance:**
- No disease claims. Use structure/function language only.
- Personal experience content (Script 4) must include "individual results may vary" disclaimer.
- See `Product/Compliance & Claims/claims-guidelines.md` for full guidelines.
