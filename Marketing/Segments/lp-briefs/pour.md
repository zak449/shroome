# Ghost LP Brief — /lp/pour (the fifteen-second crowd)

> Segment: time-starved & on-the-go (commuters, moms, travel) · See `../segment-strategy.md` §5
...
- **OG title:** `shroomé — a café matcha latte in 15 seconds. drop 002 is limited.`
- **OG description:** `tear. pour. done. ready-to-pour ceremonial matcha latte — no whisk, no blender, no line. drop 001 sold out. get drop access before 002 goes.`
- **OG image:** one-handed pour into a to-go cup (cup must read "shroomé"), keys and a bag on the counter, morning-rush light — motion, not stillness (real photography only)
...
> the café line was the only ingredient we removed.
...
**Block 2 — title:** *café-grade, not compromise-grade*
...
**Block 3 — title:** *lives wherever your day happens*
...
Body copy:
> we made [X₁] boxes for drop 001. the access list cleared them before the site ever opened to the public. no review wall yet — just a sold-out ledger, a fully disclosed label, and drop 002 on the way.
...
## 5. FAQ (accordion, 4 items — keep answers short; this audience skims)
...
**q: how do drops work?**
a: we produce in numbered, limited runs. drop 001 ([X₁] boxes) sold out; drop 002 is [X₂] boxes. drop access gets you the link at open — the text list gets it 10 minutes early, plus an extra 10% off. real runs, real dates, no fake timers.
...
- Drop-access signup → `/api/waitlist` (email step, then optional phone step for SMS early access), `utm_campaign=lp-pour`
- CTA 3x: hero, post-FAQ, final. Ghost page — no nav/exit links except footer legal; one decision, zero browsing detours. This is the most impulse-optimized page in the set: shortest total copy, biggest type, fastest scroll
...
- Hero one-take video ≤ 2MB loop, autoplay muted, `prefers-reduced-motion` fallback to mid-pour still
- Pre-ship audit: no pregnancy/nursing language, no child-facing framing, no "healthiest" superlatives
