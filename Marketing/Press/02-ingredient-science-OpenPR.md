# Press Release #2 — Ingredient Science Angle
**Target Platform:** OpenPR (openpr.com)
**Angle:** Beta-glucan science and ingredient transparency
**Status:** Ready to submit

---

**FOR IMMEDIATE RELEASE**

## New Functional Matcha Brand Spotlights Beta-Glucan Concentration Gap in Mushroom Supplement Market

*shroomé's organic mushroom extracts verified at 70%+ beta-glucan content — more than double the concentration found in most commercial mushroom products*

**LOS ANGELES, CA — March 2026** — As the functional mushroom market grows toward an estimated $19.3 billion globally, a new entrant is drawing attention to a significant gap in product quality. shroomé (drinkshroome.com), a ready-to-pour ceremonial matcha latte brand from ZSQUARED INC, has formulated its mushroom extract component at 70%+ verified beta-glucan concentration — a number that highlights how much variation exists across the category.

Beta-glucans are polysaccharides found in the cell walls of fungi that have been the subject of extensive research regarding immune function support. However, the concentration of beta-glucans in commercial mushroom products varies dramatically depending on source material and extraction methods.

**Fruiting Body vs. Mycelium-on-Grain: Why It Matters**

The distinction between fruiting body extracts and mycelium-on-grain products is a central issue in mushroom supplement quality. Fruiting body extracts are derived from the mature mushroom itself, where beta-glucan compounds are most concentrated. Mycelium-on-grain products, by contrast, grow fungal mycelium on a grain substrate (typically rice or oats), and the final product often contains significant amounts of starch from that substrate — diluting the active compounds.

shroomé uses exclusively fruiting body extraction, followed by hot water processing and spray drying, to produce a concentrated powder that third-party testing has verified at 70%+ beta-glucan content. The company publishes its full ingredient list with exact dosages — no proprietary blends.

"Consumers deserve to know what concentration of active compounds they are actually getting," said Zachary Kaufman, founder and CEO of ZSQUARED INC. "A label that says 'mushroom extract' without specifying beta-glucan content is not providing enough information for an informed decision."

**Three Functional Ingredients in One Format**

Rather than positioning itself as a supplement, shroomé is formulated as a daily beverage — a ceremonial matcha latte that happens to contain functional ingredients. Each single-serve sachet includes:

- **2g ceremonial-grade matcha** (first-harvest, shade-grown minimum 21 days) providing approximately 50mg caffeine and naturally occurring L-theanine
- **200mg organic mushroom extracts** standardized to 70%+ beta-glucans via fruiting body extraction
- **2g grass-fed collagen peptides** (hydrolyzed, Types I and III) from pasture-raised bovine sources

The product contains no added sugar, no artificial sweeteners, no artificial colors, and no preservatives. All ingredients are tested for heavy metals (arsenic, lead, cadmium, mercury) and microbial contamination, with manufacturing in a GMP-certified facility.

**Consumer Transparency as Brand Principle**

shroomé publishes exact ingredient amounts per sachet rather than using proprietary blends — a practice that obscures individual ingredient doses behind a single combined weight. This approach allows consumers and practitioners to evaluate the formulation against published research on individual ingredients.

The brand is currently in pre-launch, accepting waitlist signups at drinkshroome.com ahead of direct-to-consumer availability. Two flavors — Vanilla and Strawberry — will launch in 12-sachet boxes.

**About ZSQUARED INC**

ZSQUARED INC is a Los Angeles-based functional beverage company. Its first product, shroomé, combines ceremonial matcha, organic mushroom extracts, and grass-fed collagen in a convenient single-serve format. For more information, visit drinkshroome.com.

**Contact:**
hello@drinkshroome.com
drinkshroome.com

*These statements have not been evaluated by the Food and Drug Administration. This product is not intended to diagnose, treat, cure, or prevent any disease.*

---

### Submission Notes:
- **Platform:** OpenPR (openpr.com) — free tier, 1 submission/month, max 1000 words
- **Category:** Health & Medicine / Food & Beverages
- **Tags:** beta-glucan, mushroom extract, functional food, matcha, ingredient transparency, clean label
- **Location:** Los Angeles, CA
- **Include link:** drinkshroome.com
- **Note:** OpenPR has manual editorial review — keep claims factual and avoid superlatives
